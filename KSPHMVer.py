...
def loadData(dir,name):
    ...
    if name == 'misalign':
        return io.loadmat(dir)['misalign']
    if name == 'inner':
        return io.loadmat(dir)['inner']
    if name == 'outer':
        return io.loadmat(dir)['outer']
    if name == 'unbalance':
        return io.loadmat(dir)['unbalance']

def appendExtractedFeatureArr(lst,infoLst,channel):

    rawArr, idx = infoLst[0],infoLst[1]

    ExtracedFeatureArr = getFeatureExtraction(rawArr[:,channel])
...
def ExceuteFeatureExtractionforAllArr(dir,loadName,channel,test):

    loadedRawArr = loadData(dir,name=loadName)
    totalIdx = int(len(loadedRawArr)/10)
    print(f'totalIdx is :{totalIdx}')


    if test== 'ver1':

        totalArr = []
        for eachIdx in range(totalIdx):

            FeatureExtracted = getFeatureExtraction(loadedRawArr[eachIdx*10:(eachIdx+1)*10,:])

            totalArr.append(FeatureExtracted)
            print(f'{eachIdx}/{totalIdx} complete')
    ...
    if test == 'ver2':
        ...
        doLst = []
        for eachIdx in range(totalIdx):
            doLst.append([loadedRawArr[eachIdx*10:(eachIdx+1)*10,:],eachIdx])
...
def plotEachFeature(arr,saveDir):
...
def saveResults(DataDir,
    ...
    rawArr = ExceuteFeatureExtractionforAllArr(DataDir,loadName=name,channel=channel,test='ver2')
...
baseDir = '/home/a286winteriscoming/Downloads/KSPHM/'
fleLst = [['innerrace_fault','inner'],['misalign','misalign'],['outerrace_fault','outer'],['unbalance','unbalance']]

for i in fleLst:
    saveResults(DataDir=baseDir+i[0]+'.mat',
                FESaveDir=baseDir,
                PCASaveDir=baseDir,
                csvSaveDir=baseDir,
                name=i[1])
