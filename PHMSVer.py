...
def loadData(dir):
    ...
    with open(dir,'r') as f:
        rdr =csv.reader(f)
        totalLst = list(rdr)
    lstWithoutHeader = totalLst[1:]

    fianlArr = np.array(lstWithoutHeader).astype('float')

    return fianlArr

def appendExtractedFeatureArr(lst,infoLst):

    rawArrDir, idx = infoLst[0],infoLst[1]

    loadedRawArr = loadData(rawArrDir)

    ExtracedFeatureArr = getFeatureExtraction(loadedRawArr)

    lst.append([idx,ExtracedFeatureArr])

    print(f'{idx} th thing complete')



def ExceuteFeatureExtractionforAllArr(dir,test):
    ...
    if test== 'ver1':
        ...
        for idx,eachFileDir in enumerate(fileLst):
            ...
            FeatureExtracted = getFeatureExtraction(loadedData)
    ...
    if test == 'ver2':
        ...
        with ProcessPoolExecutor(max_workers=16) as executor:
            ...
            for result in executor.map(appendExtractedFeatureArr,repeat(totalArr),doLst):
                pass
        for i in totalArr:
            print(totalArr[0])

        totalArr = sorted(totalArr,key= lambda x : x[0])
        totalArr2 = []
        for i in totalArr:
            totalArr2.append(i[1])
...
def saveResults(DataDir,
    ...
    rawArr = ExceuteFeatureExtractionforAllArr(DataDir,test='ver2')

    pcaSolver,pcaArr= doPCA(rawArr,3)

    coeffMatrix = pcaSolver.components_

    plotEachFeature(rawArr, saveDir=FESaveDir +name+ 'FE.png')
    plotPCA3dScatter(pcaArr, saveDir=PCASaveDir +name + 'PCA.png')
    np.savetxt(csvSaveDir+name+'coeff.csv',coeffMatrix,delimiter=',')



baseDir = '/home/a286winteriscoming/Downloads/PHM Society/2017data_challenge_0/'

DirLst = ['training','testing']
